...
| `agents/` | Custom subagents | Specialized personas (`building-blocks/` and `orchestrators/`) |
| `skills/` | Agent skills | Multi-step workflows for autonomous execution |
| `hooks/` | Agent hooks | Deterministic shell scripts (formatting, linting, testing) |
...
./install.sh -m "instructions,rules,agents,skills,hooks" /path/to/project
```
...
Agents are split into two categories: Orchestrators (decision makers) and Building Blocks (doers).

| File | What it does |
|------|-------------|
| `agents/orchestrators/architect.md` | System design and technology selection |
| `agents/building-blocks/code-reviewer.md` | Review code for correctness, security, performance |
| `agents/building-blocks/test-writer.md` | Generate comprehensive tests with edge cases |
| `agents/building-blocks/refactorer.md` | Safely refactor code while preserving behavior |
| `agents/building-blocks/documenter.md` | Generate JSDoc/Javadoc/docstrings and API docs |
| `agents/building-blocks/debugger.md` | Diagnose and fix bugs systematically |
| `agents/building-blocks/performance-optimizer.md` | Analyze performance bottlenecks and suggest optimizations |

### Skills

Agent Skills provide Copilot with multi-step workflows.

| File | What it does |
|------|-------------|
| `skills/dev-workflow/SKILL.md` | TDD workflow state machine for feature development |
| `skills/increase-coverage/SKILL.md` | Analyzes coverage and generates targeted test files |
| `skills/perf-audit/SKILL.md` | Performs a parallel repository-wide performance audit |
| `skills/security-audit/SKILL.md` | Systematically checks for OWASP vulnerabilities |

### Hooks

Agent Hooks run deterministic bash scripts at specific points during Copilot's loop.

| File | Copilot Hook Trigger | What it does |
|------|-----------------------|--------------|
| `hooks/format-on-save.sh` | `PostToolUse` (`Edit`, `Write`) | Auto-formats code generated by Copilot using prettier/black/gofmt |
| `hooks/lint-before-commit.sh`| `PreToolUse` (`Bash` `git commit`) | Prevents Copilot from committing code with lint errors |
| `hooks/test-before-push.sh` | `PreToolUse` (`Bash` `git push`) | Prevents Copilot from pushing if tests fail |

> **Note on Automated Configuration**: When installing `hooks` to a specific project via `./install.sh /path/to/project` (or `install.ps1`), the installer will automatically configure your project's `.vscode/settings.json` to wire up Copilot's `github.copilot.advanced.hooks` settings.
...
- **Agents** — `.md` files in `agents/building-blocks/` or `agents/orchestrators/`
- **Skills** — Directories in `skills/` containing a `SKILL.md`
